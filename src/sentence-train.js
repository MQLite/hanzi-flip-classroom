...
export function createTrainView(host,dispatch,onFallback){
 ...
 const $=s=>host.querySelector(s);let key='',scene,lastPalette;
 ...
 if(key!==nextKey||lastPalette!==palette){key=nextKey;lastPalette=palette;$('#train-palette').replaceChildren(...palette.map((tile,n)=>tileButton(tile,n,'候车区')));$('#train-selected').replaceChildren();$('.train-track-scroll').scrollLeft=0}
 ...
 scene.update({palette:active?palette:[],selections:active?selections:[],key});
